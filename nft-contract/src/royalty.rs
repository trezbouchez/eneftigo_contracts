...
impl NonFungibleTokenCore for Contract {
    ...
    fn nft_payout(&self, token_id: String, balance: U128, max_len_payout: u32) -> Payout {
		...
		for (k, v) in royalty.iter() {
            // get the key
			let key = k.clone();
            // skip token owner (we add their payout at the end)
			if key != owner_id {
		...
		// payout to the seller (gets 100% - total perpetual royalties)
    ...
    fn nft_transfer_payout(
        ...
        // get the u128 version of the passed in balance (which is U128)
        let balance_u128 = u128::from(balance);
        ...
        for (k, v) in royalty.iter() {
            // get the key
            let key = k.clone();
            // skip previous token owner (we add their payout at the end)
        ...
        payout_object.payout.insert(previous_owner_id, royalty_to_payout(10000 - total_perpetual, balance_u128));
